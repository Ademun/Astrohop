...
const formattedTime = computed(() =>
  dateFormatter.format(new Date(props.missionData.time)),
);
...
// print stylesheet, which mirrors whichever mode is active on screen at
// print time — print is meant to be a faithful copy of the viewport, not a
// separately-styled artifact. Night intentionally points at the app's own
// theme tokens so the chart reads as part of the shell rather than a
// pasted-in rectangle; Day and Red are fixed literal values since neither
// should move if the app theme does.
// ---------------------------------------------------------------------------
...
/**
 * Builds a fully self-contained stylesheet for the print document. This has
 * to be self-contained: `svgEl.outerHTML` is cloned into a brand-new HTML
 * document that has none of the on-screen page's ancestors, so anything
 * that depended on inheriting `--sm-*` from the component root (as the
 * on-screen page does via `paletteStyle`) would silently resolve to nothing
 * there and fall back to black — which is exactly what made the previous
 * version of this function render an all-black page with everything drawn
 * in "ink" invisible against it. Defining the same custom properties at
 * `:root` here, and reusing the *exact* rule bodies the scoped on-screen
 * `<style>` block uses (just without the Vue scoping hash), means print is
 * always a faithful copy of whatever's on screen — same palette, same
 * selection/measurement state — with only typography swapped for a
 * classic print-chart serif/sans pairing instead of the app's UI fonts.
 */
function buildPrintStyles(p: StarMapPalette): string {
  return `
    ...
    .page-bg { fill: var(--sm-bg); }
    .page-frame { fill: none; stroke: var(--sm-frame); stroke-width: 0.9; }
    .page-title { font: 700 7px Georgia, 'Times New Roman', serif; letter-spacing: 0.3px; fill: var(--sm-ink); }
    .grid-dashed circle, .grid-dashed line { stroke: var(--sm-grid); stroke-width: 0.15; stroke-dasharray: 0.6, 0.8; }
    .axis-line { stroke: var(--sm-ink); stroke-width: 0.2; opacity: 0.5; }
    .horizon-circle { fill: none; stroke: var(--sm-ink); stroke-width: 0.5; }
    .alt-label, .az-label { font: 400 2.2px Arial, Helvetica, sans-serif; fill: var(--sm-muted); }
    .cardinal-label { font: 700 4px Georgia, 'Times New Roman', serif; fill: var(--sm-ink); text-anchor: middle; dominant-baseline: middle; }
    .route-line { stroke: var(--sm-ink); stroke-width: 0.3; stroke-dasharray: 1.3, 1; opacity: 0.85; }
    .route-line.is-active { stroke-width: 0.6; opacity: 1; }
    .distance-label { font: 400 2.6px Arial, Helvetica, sans-serif; fill: var(--sm-ink); paint-order: stroke; stroke: var(--sm-halo); stroke-width: 1px; stroke-linejoin: round; }
    .distance-label.is-active { font-weight: 700; }
    .star-glyph { fill: var(--sm-ink); transition: none; }
    .star-glyph.is-hovered, .star-glyph.is-selected, .star-glyph.is-measuring { fill: var(--sm-accent); }
    .star-ring { fill: none; stroke: var(--sm-accent); stroke-width: 0.3; }
    .star-label { font: 400 2.8px Georgia, 'Times New Roman', serif; fill: var(--sm-ink); }
    .star-label.is-hovered, .star-label.is-selected, .star-label.is-measuring { fill: var(--sm-accent); font-weight: 700; }
    .measure-line { stroke: var(--sm-accent); stroke-width: 0.4; stroke-dasharray: 0.6, 0.6; }
    .measure-label { font: 700 3px Arial, Helvetica, sans-serif; fill: var(--sm-accent); text-anchor: middle; paint-order: stroke; stroke: var(--sm-halo); stroke-width: 1px; }
    .print-caption { font: 400 3px Arial, Helvetica, sans-serif; fill: var(--sm-muted); }
    .print-caption-strong { font: 700 3px Arial, Helvetica, sans-serif; fill: var(--sm-ink); }
  `;
}
...
const routeOrder = computed(() =>
  workingOrder.value.filter((i) => points.value[i] != null),
);
  ...
  for (const request of requests) {
    ...
    if (!chosen) {
      ...
      chosen = {
        ...
        rect: rectFor(x, y, width, height, fallback.anchor),
      };
    }
...
const measurement = computed(() => {
  ...
  return {
    from,
    to,
    degrees: angularSeparationDeg(from.alt, from.az, to.alt, to.az),
  };
});
...
function onToolbarKeydown(event: KeyboardEvent): void {
  ...
  if (event.key === "ArrowRight")
    nextIndex = (toolbarFocusIndex.value + 1) % buttons.length;
  else if (event.key === "ArrowLeft")
    nextIndex = (toolbarFocusIndex.value - 1 + buttons.length) % buttons.length;
...
// second page. Renders with whichever color mode is active right now, and
// includes any current hover/selection/measurement state — it's meant to be
// a copy of what's on screen, not a separately-normalized artifact.
// ---------------------------------------------------------------------------
...
function triggerPrint(): void {
  ...
  const printStyles = buildPrintStyles(COLOR_PALETTES[colorMode.value]);

  printWindow.document.open();
  printWindow.document.write(
    `<!doctype html><html><head><meta charset="utf-8" /><title>${props.title} — star map</title>` +
      `<style>${printStyles}</style></head><body>${svgEl.outerHTML}</body></html>`,
  );
    ...
    <Card size="sm" class="w-full shrink-0 gap-4 lg:w-72">
      ...
      <CardContent class="flex flex-col gap-4 text-sm">
        <div class="grid grid-cols-2 gap-3">
          <div>
            ...
            <p class="mt-0.5 font-medium">
              {{ missionData.location.lat.toFixed(2) }},
              {{ missionData.location.long.toFixed(2) }}
            </p>
          </div>
          <div>
            <p class="text-xs uppercase tracking-wide text-muted-foreground">
            ...
            <p class="mt-0.5 font-medium">
              {{ missionData.conditions.limiting_magnitude }}
            </p>
          </div>
        </div>
          ...
          <ul class="mt-1.5 flex max-h-56 flex-col gap-1 overflow-y-auto">
            <li v-for="pt in orderedDisplayPoints" :key="pt.key">
              <button
                ...
                :class="
                  selectedIndices.has(pt.index) ||
                  measureIndices.includes(pt.index)
        ...
        >
          <span class="flex items-center gap-1.5">
            <span
              ...
              style="background: var(--sm-ink)"
            />
            Star
          </span>
          <span class="flex items-center gap-1.5">
            <span
              class="inline-block h-px w-3 border-t border-dashed"
              style="border-color: var(--sm-ink)"
            />
            Path
          </span>
        </div>
        ...
        <div
          ...
          class="rounded-md border border-dashed border-border p-2.5 text-xs"
        >
          <template v-if="measurement">
            <p class="font-medium text-foreground">
              {{ measurement.from.name }} → {{ measurement.to.name }}
            </p>
            <p class="mt-0.5 text-muted-foreground">
              {{ measurement.degrees.toFixed(2) }}° apart
            </p>
          </template>
          <p v-else class="text-muted-foreground">
            Tap
            {{ measureIndices.length === 0 ? "two stars" : "one more star" }} to
        ...
        >
          <span class="text-muted-foreground"
            >{{ selectedIndices.size }} selected</span
          >
          <Button
            variant="ghost"
            size="xs"
          ...
          >
            ...
            <div class="flex items-center gap-1">
              ...
              <Tooltip>
                ...
                <TooltipContent
                  >Red-light mode — preserves night vision</TooltipContent
                >
              </Tooltip>
          ...
          <div class="relative">
            ...
            >
              ...
              <g v-for="(seg, k) in segments" :key="`seg-${k}`">
                ...
                <text
                  :x="
                    resolvedLabels.get(`seg:${k}`)?.x ??
                    (seg.from.x + seg.to.x) / 2
                  "
                  :y="
                    resolvedLabels.get(`seg:${k}`)?.y ??
                    (seg.from.y + seg.to.y) / 2
                  "
                  :style="{
                    textAnchor:
                      resolvedLabels.get(`seg:${k}`)?.anchor ?? 'middle',
                  }"
              ...
              <g v-for="pt in points" :key="`star-${pt.key}`">
                ...
                <text
                  ...
                  :style="{
                    textAnchor:
                      resolvedLabels.get(`star:${pt.index}`)?.anchor ??
                      'middle',
                  }"
                  class="star-label"
                  :class="{
